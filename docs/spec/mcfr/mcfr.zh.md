...
**这是两个后端明知会答不一样的唯一一个字段。** build 存的是描述里的间隔加上一次**按单位
抽取的错开**，抽取范围是该单位的 `interval_offset`：偏移为 0 的单位读出来就是它的描述，
同一场仗里三只长弓读出 55、65、56（描述是 62），而同一份 layout 里的同一只单位在任何对局
种子下都抽到同一个数。[`combat.md`](../../rules/combat.zh.md) 记着这次测量。模拟器是把自己
的错开排到"下一次攻击的步数"上而不是存起来，所以两边对"何时开火"一致、对"间隔读作多少"
不一致——这是**表示上的差异而不是行为上的差异**，而内容层正是这种东西该浮出来的地方。
