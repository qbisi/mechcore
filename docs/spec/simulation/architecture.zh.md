...
**执行者还没决定的，就在一个地方拒绝。** 瞄准建筑的一次命中，溅射到边缘在半径内的
其他敌方建筑，每个都吃满额——这是量出来的游戏行为。跨两类的还没决定：瞄准单位的命中
溅射若会波及建筑，或瞄准建筑的命中溅射若会波及单位，不管是谁打出来的，都在
`damage_targets` 里拒绝，因为它打到谁、打多少，是这份契约没有携带的测量。
