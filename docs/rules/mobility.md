...
This index is pinned to game build 2259. It states which formations a side may
move during deployment, what frees a formation that may not, and the evidence
for both.

A state carries the answer per formation as `movable`;
[`state.md`](../spec/document/state.md#a-formation-carries-two-fields-a-layout-does-not)
defines the field and [`action.md`](../spec/document/action.md#move_unit) the
decisions that write it.

## The rule

A formation may move in the round it arrives, and stays where it is in every
...
Three things free a formation that has stayed from an earlier round:

| What | ID | Frees |
| --- | --- | --- |
| Deployment Module, 部署模块 | equipment `13040001` | the formation that wears it, in every round |
| Jump Drive, 高速引擎 | technology `1606` Wasp, `1611` Overlord, `1616` Phoenix | every formation of that unit, in every round |
| Redeploy, 再部署 | commander skill `1000001` | the formation it targets, for the rest of the round |
...
a formation the rule lets move:
...
A formation "delivered as the round opened" is one at or above the unit
allocator the previous round closed with. The snapshot's own `roundCount` does
not separate those from formations bought the round before: an officer's squad
delivered as a round opens can already read 1.

The corpus holds no move the game refused, so it shows the rule is not too
strict and cannot show it is not too lenient. A formation that stays in place
