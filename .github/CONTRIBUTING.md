...
| --- | --- | --- |
| the build's decompilation, one directory per build, and its symbol index as a release | [`qbisi/mechcore-decomp`](https://github.com/qbisi/mechcore-decomp), private | a cloud session through GitHub: attached to the session or through the GitHub connector in Claude Code, through a read-only token on that repository alone in Codex |
