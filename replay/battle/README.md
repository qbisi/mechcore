...
basename in `replay/grbr`, with nothing edited afterwards. They are regenerated,
...
```bash
mechcore replay convert "replay/grbr/<name>.grbr" "replay/battle/<name>.yaml" --force
```
...
`mechcore doc verify replay/battle/*.yaml` first reads each document as a stream and
