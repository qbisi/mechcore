...
`reactor_core` 在战斗里的变化同样不在部署转移的职责内。增援发牌已可独立预测，
但尚未组成统一的回合初始化转移。
...
1. 把回合开局本身写成转移：军官交付、增援发牌、补给收入都发生在 `OnEnterDeploy`
   而不是某个动作上，语料里每回合最后一次 initialization 的 after 就是它的验收
   目标。做完之后 `apply` 的九字段可以由 `step` 折叠推出，两条路径不再各算各的。
