...
**回合初始化作为转移已经闭合。** 第 0 回合由 header 构造选择前的局面（地图反应堆、
建筑、塔等级），走 `step`、队伍交付与 `open_round(1)` 同一条链；此后每回合的开局
重置与军官交付都是 `open_round`。convert 读取开局前的快照后调用同一个
`open_round`，只有补给和装备库存还由它自己的规则重建。
...
指标已经立起来，回合初始化与第 0 回合已经闭合，追踪集只剩补给未实现。接着是
补给（二），顺带初始冷却（一），然后合并（三），再把规则迁到 simulation（四）。部署末 layout 的实机批量验证、
