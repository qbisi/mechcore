...
**回合初始化作为转移。** `transition::open_round` 只做军官交付。技能冷却、`used`
与 `release` 的清理、购买与解锁额度、能量塔技能重置、`movable` 与 `travelling`
的重置、回合收入都还只在 convert 里按快照重建。convert 的开局与原生开局相等，
只说明端点重建一致，不证明"初始化之前 → 初始化之后"的转移。
...
按字段组把 convert 里的开局重建搬进 `open_round`，每搬一组，就把它移出
`coverage::UNIMPLEMENTED`，指标里该组从未实现变为一致，convert 改为调用同一条
规则，不留两份：

- `battle_skills`：清 `used` 与 `release`，冷却按 `releaseInterval` 重置或
  递减，新增技能用初始冷却，保持重复 ID 的槽位身份。
- `shop.buys_remaining` 与 `shop.unlocks_remaining`：按下一开局持有的军官重置。
- `energy_tower_skills`：开局重置，债务正确传递到补给。
- `formations.movable` 与 `formations.travelling`：开局重置，本回合到场与
  `mobility::free` 的编队可移动。
- 第 0 回合选择到 `state(1)` 的转移：先由 header 构造选择开局前的局面，
  再走同一条链，不再整段计作未实现。
...
指标已经立起来。先按字段组清零初始化（一），然后补给（二）与合并（三）。
部署末 layout 的实机批量验证、反汇编核对和原生 turn 执行按需穿插，都不是
指标的前置条件。战斗字段（四）等模拟器。
