...
- ~~**什么让单位去打墙。**~~ 量完了，规则在 `docs/rules/constructions.md`：单位
  不换锁定目标，打的是"自己到目标那条线"够得着的最近一块敌方工事，线宽 11 米、
  只看攻击距离 + 7 米以内的。**模拟器还没实现它**，所以墙摆上去了永远不会被打
  ——这是现在唯一会静默错的地方，闭包里没有回合所以还没人消费它。实现要两样：
  目标搜索之后加一道挡路检查，以及内核能对建筑开火。
