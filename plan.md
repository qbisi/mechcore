...
只剩 `predict` 一份。接着把规则迁到 simulation（一）。部署末 layout 的实机批量
验证、反汇编核对和原生 turn 执行按需穿插。战斗字段（二）等模拟器。新增技能的
初始冷却是否读表，等后续版本对闪电风暴与核弹的处理，见 state spec 的 Unresolved。
