...
**覆盖指标本身。** `verify <battle.yaml>` 只核对开局与增援发牌。battle 的 state
与 action 还不能按完整文档类型读回，也没有叶级比较和四类归属。转换时的
九字段检查和补给账本不能代替它：前者只比较部分字段，后者从后继 state 取值。
...
**系统性的部署末 layout 实机验收、执行一个 turn 的能力。** 与上一版计划相同：
...
1. battle 的 state 与 action 按完整文档类型读取，拒绝缺字段、类型错误、未知
   字段和未知动作，保留技能目标的 YAML 标签。`stash@{0}` 里有一版现成实现，
   先审再取，不整包恢复。
2. 转移预测 `predict(economy, context, state(r), actions(r))`：`step*` 折叠
   后接 `open_round(r+1)`，落位用 `landing`。没有初始化规则的字段原样透传，
   让它们如实落进不一致或未实现，而不是先把它们排除在外。
3. 叶级比较与四类归属，接进 `verify <battle.yaml>` 的报告，与已有开局和发牌
   检查放在同一份结果里；`reinforce_offers` 作为一个叶计入。输出契约同步到
   battle spec，白名单写在那里。
4. 追踪集断言钉住按字段组的四类计数，任何变化都要改断言才能过。
5. 反向用例：在测试临时数据里改掉后继 state 的一个已覆盖字段，或删改一个
   影响它的动作，报告必须在对应字段出现不一致。格式合法但数值错误不能漏检。

### 二、把初始化补齐，逐组清零

按字段组把 convert 里的开局重建搬进 `open_round`，每搬一组，指标里该组从
不一致变为一致，convert 改为调用同一条规则，不留两份：
...
- 第 0 回合选择到 `state(1)` 的转移用同一条链，不再单独特判。
...
先立指标（一），再按字段组清零初始化（二），然后补给（三）与合并（四）。
