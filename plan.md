...
- **`wall-block` 第 96 tick。** 一只在打墙的爬虫，游戏读成空闲、无锁定，模拟器还在打。
- **`wall-passage` 第 44 tick。** 放墙那方的爬虫穿自己的墙，避让速度分叉。模拟器里自己
  的墙不是邻居；游戏会不会先把它算进 20 个邻居里、再忽略，是下一个要量的。
...
- **跨类溅射。** 打墙的溅射会不会带上墙边的单位、打单位的溅射碰到墙块怎样——都没量，
  模拟器两种都拒绝。
