...
的 84 场录像里 75 场逐 tick 对上并钉住；arclight、mustang、phoenix 已达标。按构建改正过的：
旋转编队的行列数（fang 的 2 场 M5）；钢球全歼对面时塔的拆除事件（6 场），`TryDstroyTower`
不看最后一击的攻击方式；终局把败方的塔交给胜方单位的那一 tick，败方最后的单位在打塔时也
照交（stormcaller 的 2 场）。另外 9 场分在三个机制上，都不是单位自己的伤害或移动：目标
正前方时出生朝向的正负号（2 场）、fang M6 一处 0.0015 m/s 的速度差（1 场）、wraith 的分组
武器搜索（5 场，外加 1 场被模拟器按名拒绝）。逐场的 tick 与分叉点见
