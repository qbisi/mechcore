...
**部署执行器。** `apply` 只推进那七个字段。编队、移动、升级、装备和技能释放
还没有由动作从回合初推到回合末，反应堆和经验则要等模拟器。这是主线上最大的
一块，也是整份 layout 对比的前提：实机 fixture 是部署结束时的位置，
没有执行器就只能比那几个战斗不碰的字段。
...
**认输的表示。** `PAD_GiveUp` 结束一局而不是移动一个位置，`action.md` 因此
没有给它表示，代价是一份本机录像转不出来。
...
都不需要游戏，也不依赖别的工作，所以最先做。

- 装备清单进 `Settled`。等式是
  `清单(R+1) = 清单(R) + 卡牌发放 + 军官发放 − 本回合装配`；军官那一项最容易漏，
  `config/officers.yaml` 写了、`Economy::Officer` 也解析了，只是没人读。
- 装置分配器进 `Settled`。`next_index.contraption` 每次释放装置加一。
- 认输改成 battle 的结果而不是回合动作。
- 删掉 `Granted::technologies` 和 `Granted::blueprints`，两者被 `apply` 读取
  却无人写入。
- 校验录像的 `energyTowerSkills` 与账本从动作推出的债一致。账本目前没算错，
  缺的是这道交叉校验。
...
一最便宜，先做。二是主线。三按适配器的进度排。四和上面三项并行，不互相阻塞。
