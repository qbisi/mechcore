...
`match new`／`show`／`act`／`commit`，战斗由模拟器结算。

- 账本就是 battle 文档，旁边只有一个 turn 文件装锁、本回合开始时刻、两方未提交
  的决策和"谁在结算"。
- `act` 只动本方草稿，`commit` 把它缩并后写进文档，写了就不能撤回。双方都写了就
  结算；部署时限（header 里的 `deploy_time`，默认 100 秒，取自原生 `DeployTime`）
  到了还没提交的那一方直接判负，这是平台的简化规则，游戏超时怎么处理没有证据。
- `match new` 不带参数也能开：种子、地图、时限缺什么就分配什么并写进 header。
  方位是分配的不是认领的：先来的是蓝方，后来的是红方，turn 文件记下来，第三个
  进程被拒绝。
- `show --wait` 承担等待；`doc project` 随时导出某一回合的 layout，所以不留录像。

战斗那半边先写"从录像读出五个字段"的读取器：反应堆伤害、装置、地形、空投护盾
都能从 MCFR 读到。经验 MCFR 没有记，模拟器覆盖不到的局面也还很多，这两处在
`match` 里明确标成缺口并拒绝，不给近似值。
