...
**批量 oracle 语料。** 整场部署记录接口已经存在，尚未把本机语料按独立生成、
独立失败和可续跑的单元组织起来。提前认输、未完整结束部署的录像按名拒绝；
战斗过程和最后一场战斗后的状态不由部署采集器声称覆盖。
...
1. 使用 battle 级记录接口组织本机原样录像，以每份录像为独立生成与失败单元。
   记录源身份、输出身份、失败原因和续跑状态，不覆盖已有采集。
2. 逐回合切出部署样本，跨回合跳转、快照重置及未观测的结束事件不进入动作折叠。
