...
**从 seed 直接决定开局随机流的起点。** 转换器已能用录像随机状态还原双方开局
选项，`verify` 从 seed 在有限窗口内搜索能发出同一组选项的位置。增援池在开局
前消耗随机数的确切次数尚未由规则计算。发牌数量和队伍多样性阈值已从 build 2259
资源读取，比较使用的剩余次数已按原生指令核对。
