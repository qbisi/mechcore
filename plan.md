...
- **统一索敌更新（第 2 步）。** 进行中。有了游戏技能状态的采集（`tests/regression/skill-state.mcscript`，
  82 个用例）当对照，内核的技能状态已经和游戏 99% 的单位-tick 一致：技能在运动进入攻击的那个 tick
  就进入攻击／准备状态；死掉的锁定一律走 `check_attackable` 重新搜；冷却只由 `finish_attack` 开始，
  原来的"击杀后冷却保持"起点和无身体单位的待发目标切换两条旧路径删了；投射物连发期间不检查。
  **还剩：** 活着的锁定离开攻击区域仍由快速切换、失效目标、失效替换三条旧路径回答——游戏里那时
  `SearchLockTarget` 拿的是 `PreCalculate` 预先准备的选择器任务，而攻击状态只在锁定死了或为空时才准备，
  内核还没有这个任务；朝向修正中进入攻击的那一 tick 也还差一拍。可能需要 checker hook 的运行时采集。
