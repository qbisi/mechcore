...
// The build-2259 Training Ground registers these 73 locked map agents after
// the four core towers and before fight mechs. The current Adapter sidecar
// observes the same ordered Q32 position/radius triples on every sampled-RVO
// update. They do not pass ordinary unit collision masks (layer 32), but they
// split the native quadtree and therefore affect traversal and query bounds.
const TRAINING_GROUND_STATIC_RVO_AGENTS: &[(i64, i64, i64)] = &[
    (460, 0, 5),
    (470, 0, 5),
    (-355, 10, 10),
    (505, 20, 5),
...
pub(crate) fn training_ground_static_agents() -> impl Iterator<Item = AgentInput> {
    TRAINING_GROUND_STATIC_RVO_AGENTS
        .iter()
        .enumerate()
        .map(|(index, &(x, y, radius))| AgentInput {
            key: AgentKey::Static(index as u8),
            main_layer: 1,
            layer: 32,
            collides_with: 0,
            group: 8,
            ...
            radius_outer: radius * Q32_ONE,
            radius_inner: radius * Q32_ONE,
            size: AgentSizeType::M,
            // Locked targets bypass neighbour priority; this value is not
            // consumed by any movable-agent solution in the modeled path.
            priority: Q32_ONE,
        })
}
...
mod tests {
    ...
    fn frozen_three_actor_entities() -> Vec<AgentInput> {
        vec![
            observed_agent(
                AgentKey::Building(1),
                FixedVec2 {
    ...
    fn neighbour_keys(inputs: &[AgentInput], key: AgentKey) -> Vec<AgentKey> {
        let source = inputs
            .iter()
            .find(|agent| agent.key == key)
            .expect("source agent exists");
        nearest_neighbours(source, inputs)
            .into_iter()
            .map(|agent| agent.key)
            .collect()
    }
    ...
    #[test]
    fn training_ground_static_agents_restore_native_three_actor_neighbours() {
        let entities = frozen_three_actor_entities();
        assert_eq!(
            neighbour_keys(&entities, AgentKey::Unit(1)),
            vec![
                AgentKey::Building(1),
                AgentKey::Building(2),
                AgentKey::Building(3),
        ...
        assert_eq!(
            neighbour_keys(&entities, AgentKey::Unit(2)),
            vec![
                AgentKey::Unit(3),
                AgentKey::Building(3),
                AgentKey::Building(4),
        ...
        assert_eq!(
            neighbour_keys(&entities, AgentKey::Unit(3)),
            vec![
                AgentKey::Unit(2),
                AgentKey::Building(4),
                AgentKey::Building(3),
        ...
        let mut native_inputs = entities[..4].to_vec();
        native_inputs.extend(training_ground_static_agents());
        native_inputs.extend_from_slice(&entities[4..]);
        assert_eq!(
            neighbour_keys(&native_inputs, AgentKey::Unit(1)),
            vec![AgentKey::Building(1), AgentKey::Building(2)]
        );
        assert_eq!(
            neighbour_keys(&native_inputs, AgentKey::Unit(2)),
            vec![AgentKey::Unit(3)]
        );
        assert_eq!(
            neighbour_keys(&native_inputs, AgentKey::Unit(3)),
            vec![AgentKey::Unit(2)]
        );
    }
