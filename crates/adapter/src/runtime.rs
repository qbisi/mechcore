...
use mechcore_protocol::{
    ...
    RecordBattleArguments, RecordBattleInstrumentation, RecordReplayBattleArguments,
...
fn serve_client(runtime: &mut Runtime, mut stream: UnixStream) -> io::Result<()> {
    ...
    loop {
        ...
        let response = match request.operation {
            ...
            Operation::RecordReplayBattle => execute_replay_battle(runtime, &request),
...
/// Record the replay's opening and every deployment, never attributing a
/// snapshot jump to a player action. Only one round is buffered in memory.
#[allow(clippy::too_many_lines)]
fn execute_replay_battle(runtime: &mut Runtime, request: &Request) -> Response<Value> {
    let args: RecordReplayBattleArguments = match serde_json::from_value(request.arguments.clone())
    {
        Ok(args) => args,
    ...
    let prepare = || -> Result<_, String> {
        if !args.grbr.is_absolute()
            ...
            || args.output.exists()
            || args.output.extension().and_then(|s| s.to_str()) != Some("jsonl")
        {
            return Err("record_replay_battle needs an existing absolute .grbr and a new absolute .jsonl output".into());
        }
        ...
        {
            return Err("battle oracle requires a locally recorded standard 1v1 build-2259 replay without game rules".into());
        }
        let count = record.players.entries[0].rounds.entries.len();
        if count < 2
            || count > i32::MAX as usize
            || record.players.entries[1].rounds.entries.len() != count
        {
            return Err("replay needs matching opening and deployment rounds on both sides".into());
        }
        let mut expected = vec![vec![Vec::new(); 2]; count];
        let mut concessions = Vec::new();
        for (team, player) in record.players.entries.iter().enumerate() {
            for (index, round) in player.rounds.entries.iter().enumerate() {
                if usize::try_from(round.round).ok() != Some(index) {
                    return Err("replay rounds must be consecutive, starting at zero".into());
                }
                let kinds = &mut expected[index][team];
                kinds.extend(round.actions.entries.iter().map(|a| a.kind.clone()));
                if index == 0 {
                    if kinds.as_slice() != ["PAD_ChooseAdvanceTeam"] {
                        return Err("opening requires one advance-team choice per side".into());
                    }
                } else {
                    if index + 1 == count && kinds.last().is_some_and(|a| a == "PAD_GiveUp") {
                        kinds.pop();
                        concessions.push(serde_json::json!({
                            "team": team, "round": index, "native_type": "PAD_GiveUp",
                            "source_action_ordinal": kinds.len(), "observed": false,
                        }));
                    }
                    if kinds.last().is_none_or(|a| a != "PAD_FinishDeploy")
                        || kinds[..kinds.len() - 1]
                            .iter()
                            .any(|a| a == "PAD_FinishDeploy" || a == "PAD_GiveUp")
                    {
                        return Err(format!(
                            "round {index}, team {team} has no complete deployment or an unsupported early ending"
                        ));
                    }
                }
            }
        ...
        let snapshots: Vec<_> = record
            ...
            .map(|r| r.round)
            .collect();
        if snapshots != (0..i32::try_from(count).map_err(|e| e.to_string())?).collect::<Vec<_>>() {
            return Err(
                "match snapshots do not cover the same consecutive rounds as player records".into(),
            );
        }
        Ok((
    ...
    let (source_hash, expected, concessions) = match prepare() {
        Ok(value) => value,
        Err(error) => return Response::failure(request.id, "invalid_arguments", error),
    };
    let status = match successful_result(execute_internal_on_main(
        runtime,
        request.id,
        operations::InternalOperation::Status,
    )) {
    ...
    if status["status"] != "main_menu" {
        return Response::failure(
            request.id,
            "invalid_state",
            "record_replay_battle starts at main_menu",
        );
    }
    let create_output = || -> Result<_, String> {
    ...
    let mut temporary = match create_output() {
        Ok(value) => value,
        Err(error) => return Response::failure(request.id, "battle_publication_failed", error),
    };
    let run = (|| -> Result<_, Response<Value>> {
        let error = |message| Response::failure(request.id, "battle_capture_failed", message);
        let mut writer = BattleTraceWriter::new(BufWriter::new(temporary.as_file_mut()));
        writer
            .line(serde_json::json!({
                "kind": "header", "schema": "mechcore.battle-observation.v1",
                "state_encoding": "native_snapshot_and_live_board", "game_version": "1.11.1.3.2259",
                "source": {"grbr": args.grbr, "build": "2259", "blake3": source_hash},
                "rounds": expected.len() - 1, "opening_round": 0,
                "combat_observed": false,
            }))
            .map_err(error)?;
        ...
        for (index, expected) in expected.iter().enumerate() {
            if Instant::now() >= deadline || evicting() {
                return Err(error(
                    "battle observation exceeded its time budget or was evicted".into(),
                ));
            }
            let round = i32::try_from(index).map_err(|e| error(e.to_string()))?;
            if round > 0 {
                // FinishDeploy may return before its process-state transition
                // runs. Do not race the native jump against that pending entry.
                wait_layout_status(
                    runtime,
                    request.id,
                    (Instant::now() + REPLAY_LOAD_TIMEOUT).min(deadline),
                    "settled replay before round jump",
                    LAYOUT_DEPLOYMENT_STABLE_SAMPLES,
                    |status| {
                        if round == 1 {
                            is_replay_state(status, 1, true, false)
                        } else {
                            is_replay_state(status, round - 1, false, true)
                        }
                    },
                )?;
            }
            successful_result(execute_internal_on_main(
                runtime,
                request.id,
                operations::InternalOperation::StartDeploymentCapture(round),
            ))?;
            if round == 0 {
                successful_result(execute_replay_load_on_main(
                    runtime, request.id, &args.grbr, 0,
                ))?;
            }
            let observation =
                collect_deployment_round(runtime, request.id, round, deadline).map_err(|mut response| {
                    if let Some(error) = &mut response.error {
                        error.message = format!("round {round}: {}", error.message);
                    }
                    response
                })?;
            validate_deployment_coverage(&observation, expected)
                .map_err(|e| error(format!("round {round}: {e}")))?;
            let events = observation["events"]
                .as_array()
                .ok_or_else(|| error("missing events".into()))?;
            let entry = &events[0]["before"];
            if round > 0 {
                writer
                    .line(serde_json::json!({
                        "kind": "round_jump", "from_round": round - 1, "to_round": round,
                        "native_type": "ReplayMatchGotoCommand.Execute",
                        "before": previous, "after": entry,
                        "combat_skipped": round > 1, "combat_observed": false,
                        "action_effect": false,
                    }))
                    .map_err(error)?;
            }
            writer
                .line(serde_json::json!({
                    ...
                    "phase": if round == 0 { "opening" } else { "deployment" }, "state": entry,
                }))
                .map_err(error)?;
            for event in events {
                let mut event = event.clone();
                event["round"] = serde_json::json!(round);
                writer.line(event).map_err(error)?;
            }
            let count = observation["actions"]
                .as_u64()
                .ok_or_else(|| error("missing actions".into()))?;
            actions += count;
            previous = observation["terminal"].clone();
            writer.line(serde_json::json!({
                "kind": "round_end", "round": round, "events": events.len(),
                "actions": count, "terminal": previous,
                "boundary": if round == 0 { "after_opening_choices" } else { "before_final_finish_deploy" },
            })).map_err(error)?;
        }
        writer
            .line(serde_json::json!({
                "kind": "battle_end", "round": expected.len() - 1,
                "reason": "source_exhausted", "terminal": previous,
                "post_combat_state": null, "combat_observed": false,
                "source_concessions": concessions,
            }))
            .map_err(error)?;
        let records = writer.sequence;
        writer
            .line(serde_json::json!({
                "kind": "summary", "complete": true, "rounds": expected.len() - 1,
                "records": records + 1, "actions": actions,
                "source_actions": actions + concessions.len() as u64,
                "unobserved_source_actions": concessions.len(),
                "terminal": previous,
            }))
            .map_err(error)?;
        writer.writer.flush().map_err(|e| error(e.to_string()))?;
        Ok((
            writer.hash.finalize().to_hex().to_string(),
            actions,
            records + 1,
        ))
    })();
    ...
    let (hash, actions, records) = match run {
        Ok(result) => result,
    ...
    let publish = || -> Result<(), String> {
        let bytes = fs::read(&args.grbr).map_err(|e| e.to_string())?;
        if blake3::hash(&bytes).to_hex().as_str() != source_hash {
            return Err("source replay changed during battle observation".into());
        }
        temporary.as_file().sync_all().map_err(|e| e.to_string())?;
        if evicting() {
            return Err("evicted before battle publication".into());
        }
        temporary
    ...
    match publish() {
        Ok(()) => Response::success(
            request.id,
            serde_json::json!({
                "recorded": true, "output": args.output, "rounds": expected.len() - 1,
                "actions": actions, "records": records,
                "unobserved_source_actions": concessions.len(),
                "blake3": hash, "source_blake3": source_hash,
            }),
        ),
        Err(error) => Response::failure(request.id, "battle_publication_failed", error),
    }
}

fn collect_deployment_round(
    runtime: &mut Runtime,
    request_id: u64,
    round: i32,
    battle_deadline: Instant,
) -> Result<Value, Response<Value>> {
    let deadline = (Instant::now() + REPLAY_LOAD_TIMEOUT).min(battle_deadline);
    // The observer can finish while the native loader is still returning.
    loop {
        if evicting() {
            return Err(evicted_response(request_id));
        }
        ...
        if round == 0 && status["status"] == "replay"
            || is_replay_state(&status, round, true, false)
        {
            successful_result(execute_internal_on_main(
                runtime,
                request_id,
...
/// Global sequence numbers include boundaries and the final summary; per-round
/// event ordinals reset at each `round_start`. Hash exactly the bytes published.
struct BattleTraceWriter<W> {
    writer: W,
    hash: blake3::Hasher,
    sequence: u64,
}

impl<W: Write> BattleTraceWriter<W> {
    ...
    fn line(&mut self, mut value: Value) -> Result<(), String> {
        value["sequence"] = serde_json::json!(self.sequence);
...
fn validate_deployment_coverage(
    ...
    let opening = observation["round"] == 0;
    if opening {
        if events
            .first()
            .is_none_or(|event| event["kind"] != "opening_start")
        {
            return Err("opening has no initial checkpoint".into());
        }
        let mut actual: [Vec<String>; 2] = [Vec::new(), Vec::new()];
        for (ordinal, event) in events.iter().enumerate() {
            if event["ordinal"].as_u64() != Some(ordinal as u64)
                || !event["before"].is_object()
                || !event["after"].is_object()
                || event["before"]["board"]["round"] != 0
                || event["after"]["board"]["round"] != 0
                || (ordinal > 0 && events[ordinal - 1]["after"] != event["before"])
            {
                return Err("broken opening state chain".into());
            }
            if ordinal == 0 || event["kind"] == "between_actions" {
                continue;
            }
            let team = event["team"]
                .as_u64()
                ...
                .filter(|team| *team < 2)
                .ok_or("invalid opening team")?;
            if event["kind"] != "action"
                || event["native_type"] != "PAD_ChooseAdvanceTeam"
                || event["accepted"] != true
                || !event["action"].is_object()
                || event["action_ordinal"].as_u64()
                    != Some(actual.iter().map(Vec::len).sum::<usize>() as u64)
            {
                return Err("invalid opening action".into());
            }
            actual[team].push("PAD_ChooseAdvanceTeam".into());
        }
        if actual.as_slice() != expected
            || observation["actions"] != 2
            || events
                .last()
                .is_none_or(|event| event["after"] != observation["terminal"])
        {
            return Err("opening coverage or terminal mismatch".into());
        }
        return Ok(());
    }
    if events
        .first()
        .is_none_or(|event| event["kind"] != "initialization")
    {
        return Err("trace does not start with round initialization".into());
    }
    let mut actual: [Vec<String>; 2] = [Vec::new(), Vec::new()];
    let mut finished = [false; 2];
    let mut action_ordinal = 0;
    let round = observation["round"]
        .as_i64()
        .filter(|round| *round > 0)
        .ok_or("invalid deployment round")?;
    for (ordinal, event) in events.iter().enumerate() {
        let last = ordinal + 1 == events.len();
        if event["ordinal"].as_u64() != Some(ordinal as u64)
            || !event["before"].is_object()
            || (!last && !event["after"].is_object())
            || event["before"]["board"]["round"]
            ...
            || (ordinal > 0 && events[ordinal - 1]["after"] != event["before"])
        {
            return Err(format!("broken state boundary at event {ordinal}"));
        }
        match event["kind"].as_str() {
            Some("initialization") if action_ordinal == 0 && !last => continue,
            Some("replay_reset")
                if ordinal > 0
                    && action_ordinal == 0
                    && !last
                    && event["action_effect"] == false =>
            {
                continue;
            }
            Some("between_actions") if ordinal > 0 && !last => {
                if event["before"] == event["after"] {
                    return Err(format!("unchanged gap at event {ordinal}"));
                }
                continue;
            }
            Some("action" | "finish_deploy") => {}
            _ => return Err(format!("unexpected event kind at {ordinal}")),
        }
        if event["action_ordinal"].as_u64() != Some(action_ordinal)
            || event["accepted"] != true
            || !event["action"].is_object()
        {
            return Err(format!("missing or rejected action at event {ordinal}"));
        }
        action_ordinal += 1;
        let team = event["team"]
            .as_u64()
        ...
        let finish = event["native_type"] == "PAD_FinishDeploy";
        if finished[team] || finish != (event["kind"] == "finish_deploy") {
            return Err(format!("invalid finish boundary at event {ordinal}"));
        }
        if finish {
            finished[team] = true;
        }
        if last
            && (!finished.iter().all(|done| *done)
                || !event["after"].is_null()
                || observation["terminal"] != event["before"])
        {
            return Err("trace does not end at the final pre-fight boundary".into());
        }
        actual[team].push(
            event["native_type"]
                .as_str()
    ...
    if !finished.iter().all(|done| *done) || observation["actions"] != action_ordinal {
        return Err("trace has incomplete action or finish coverage".into());
    }
