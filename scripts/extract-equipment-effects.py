...
outside this table. Q32.32 integers are never converted through float.
"""
...
def render(rows):
    lines = ["schema: mechcore.equipment_effects", "game_build: 1.11.1.3.2259", "",
             "# Generated by scripts/extract-equipment-effects.py from level0 path 188.",
             "# Rates and fractional values are raw Q32.32; speed is an integer.",
             "# All serialized effect and lifetime fields are retained, including zero.",
             "equipment:"]
    for d in rows:
        for i, (key, value) in enumerate(d.items()):
            lines.append(f"{'  - ' if i == 0 else '    '}{key}: " + json.dumps(value, ensure_ascii=False))
    return "\n".join(lines) + "\n"
